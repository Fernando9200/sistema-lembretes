...
const ReminderCard: React.FC<ReminderCardProps> = ({ reminder, onToggle, onRemove, onUpdate }) => {
    // ... (O código do ReminderCard permanece o mesmo)
      ...
      <div className="flex h-screen bg-gray-50">
        <aside className="w-64 bg-white border-r border-gray-200 flex-col hidden sm:flex">
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {isSaving && (
                <div className="px-4 mt-4 text-xs text-blue-600 flex items-center space-x-1.5">
                    <div className="w-2 h-2 border border-blue-500 rounded-full animate-spin"></div>
                    <span>Salvando...</span>
                </div>
            )}
          </div>
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto">
            {/* O resto do código da main continua o mesmo */}
                ...
                <div className="bg-white rounded-xl shadow-lg p-6 mb-8 border border-gray-200">
                    {isAdding ? (
                        <div className="space-y-4 animate-fade-in">
                            ...
                            <input type="text" placeholder="Título do lembrete (ex: Pagar a conta de luz)" value={newReminder.title} onChange={(e) => setNewReminder({ ...newReminder, title: e.target.value })} className="w-full p-3 border rounded-lg focus:ring-2 border-gray-300 focus:ring-primary-400"/>
